...
def prepare_mariadb_commands(database_name, database_username, database_password):
    password_option = f"-p'{database_password}'" if database_password else ""
    return [
        f"docker exec central-mariadb mysql -u root {password_option} -e 'DROP DATABASE IF EXISTS {database_name};'",
        f"docker exec central-mariadb mysql -u root {password_option} -e 'CREATE DATABASE {database_name};'",
        f"docker exec central-mariadb mysql -u root {password_option} -e 'GRANT ALL PRIVILEGES ON {database_name}.* TO {database_username};'"
    ]
...
def main(database_type, database_name, database_username, database_password=None):
    ...
    if database_type.lower() == "postgres":
        ...
        commands = prepare_mariadb_commands(database_name, database_username, database_password)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("database_type", choices=["postgres", "mariadb"], help="Type of the database (postgres or mariadb)")
